...
class AboutMultipleInheritance(Koan):
    ...
    def test_normal_methods_are_available_in_the_object(self):
        jeff = self.Spiderpig()
        self.assertRegex(jeff.speak(), "This looks like a job for Spiderpig!") #returns first speak on the spiderpig object. 
    ...
    def test_base_class_methods_can_affect_instance_variables_in_the_object(self):
        ...
        jeff.set_name("Rover")
        self.assertEqual("Rover", jeff.name) #accessed the set_name method on the original to change it to "Rover"

    def test_left_hand_side_inheritance_tends_to_be_higher_priority(self):
        jeff = self.Spiderpig()
        self.assertEqual('pink', jeff.color())
        #'black'is on spider, pink is from pig. though I'm not sure what makes the pig left hand inheritance. Oh, I guess left hand in the order they were passed into the Spiderpig (Pig, Spider, Nameable)

    def test_super_class_methods_are_higher_priority_than_super_super_classes(self):
        jeff = self.Spiderpig()
        self.assertEqual(8, jeff.legs())

        #from Spider which is super, whereas Animal is two removed. super, super class

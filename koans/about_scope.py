...
class AboutScope(Koan):
    ...
    def test_dog_is_not_available_in_the_current_scope(self):
        with self.assertRaises(NameError): fido = Dog() #dog is not defined
    ...
    def test_you_can_reference_nested_classes_using_the_scope_operator(self):
        ...
        self.assertEqual(False, type(fido) == type(rover))  #not totatlly sure why the type is different. I guess because they are made from two different classes. 
        self.assertEqual(False, jims.Dog == joes.Dog) #also not the same 
    ...
    def test_str_without_self_prefix_stays_in_the_global_scope(self):
        self.assertEqual(True, str == type("HI")) # because they are both strings. 
    ...
    def test_incrementing_with_local_counter(self):
        ...
        self.assertEqual(False, counter == start + 1) #I supposed because it can't access the result of this variable since it isn't global. And the result doesn't update the 'globally' available counter in the local counter function afterwards. 
